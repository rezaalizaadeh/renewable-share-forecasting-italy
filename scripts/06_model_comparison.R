...
best_models_by_family <- final_model_comparison %>%
  ...
  mutate(
    model_family = factor(
      model_family,
      levels = c("Baseline", "ARIMA-family", "Regression/ML")
    )
  ) %>%
...
p26 <- ggplot(
  ...
  labs(
    ...
    y = NULL,
    fill = "Model family",
    caption = "Elastic Net is evaluated in a one-step-ahead operational setup; baseline and ARIMA-family models are full test-year forecasts."
  ) +
  theme_project() +
  theme(
...
p27 <- ggplot(forecast_plot_data, aes(x = date, y = renewable_share)) +
  ...
  labs(
    ...
    subtitle = "Elastic Net is one-step-ahead; TSLM and Logit Fourier ARIMA are full test-year forecasts",
    x = NULL,
    y = "Renewable share",
    colour = NULL,
    linewidth = NULL,
...
p30 <- ggplot(
  ...
  labs(
    ...
    y = NULL,
    fill = "Model family",
    caption = "The ML comparison uses lagged information available up to the previous day and should be interpreted as operational short-term forecasting."
  ) +
  theme_project() +
  theme(
