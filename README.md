...
The goal is not just to get the lowest error. I also wanted the forecasting setup to be clear, especially because the models are evaluated under two different conditions: full test-year forecasting and one-step-ahead operational forecasting.
...
The pipeline is split into six scripts.
...
Forecast accuracy alone is not enough, so I also checked whether the residuals still contained serial dependence.
